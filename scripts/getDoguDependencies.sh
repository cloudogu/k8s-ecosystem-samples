...
if [ -z "$1" ]; then
  echo "Usage: ./getLatestDoguVersion.sh <NAMESPACE/DOGU>"
  exit 1
fi
...
getDoguDependencies() {
  ...
  DYNAMIC_PART="$1"
  FULL_URL="${BASE_URL}${DYNAMIC_PART}"

  AUTH_HEADER="Authorization: Basic $(echo -n "$DCC_USERNAME:$DECODED_PASSWORD" | base64)"
  curl -s -X GET "$FULL_URL" -H "$AUTH_HEADER" | jq '{Dependencies: .Dependencies, OptionalDependencies: .OptionalDependencies}'
}
